...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //String str = "ouii ouii ouii nonn";
        ...
        for (int i = 0; i < phrases.length; i++) {
            ...
            for (int k = 0; k < words.length; k++){
                for (int j = 0; j < words.length; j++){
                    if (j != k && !words[k].equals(words[j]) && words[k].length() > 2 && !bib.isUseless(words[k]) && words[j].length() > 2 && !bib.isUseless(words[j])) {
                        nodeHashMap.get(words[k]).addNeihboor(nodeHashMap.get(words[j]));
                    }
                }
            }
        }
        ...
        for (String word : nodeHashMap.keySet()
            ...
            list.get(nodeHashMap.get(word).getValue() - 1).add(nodeHashMap.get(word)); //sort words by their values
        }
        for (int i = 0; i < list.size(); i++){
            String strr = i + 1 + " : ";
            for (int k = 0; k < list.get(i).size(); k++){
                strr += ("{" + list.get(i).get(k).getString() + " " + list.get(i).get(k).getMaxNeighboor().getString() + "}");
            }
